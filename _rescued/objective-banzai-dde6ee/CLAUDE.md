...
### Supabase table grant rules
- **Any new table must include explicit GRANT statements** — as of May 30, 2026, new Supabase projects no longer auto-expose tables to the Data API; enforced on all existing projects October 30, 2026.
- When creating a new table, always run alongside it:
  ```sql
  GRANT SELECT ON public.<table> TO anon;          -- if anon reads it
  GRANT INSERT ON public.<table> TO anon;          -- if anon writes it
  GRANT ALL ON public.<table> TO service_role;
  ```
- Current grants verified 2026-05-27: `anon` has SELECT on `component_specs`, INSERT on `spec_submissions`.
